...
Juliana Spitzner
Data: 
Boston Uber and Lyft Rideshare Data
Description:
...
listofhours = hour0, hour1, hour2, hour3, hour4, hour5, hour6, hour7, hour8, hour9, hour10, hour11, hour12, hour13, hour14, hour15, 
hour16, hour17, hour18, hour19, hour20, hour21, hour22, hour23
 
return listofhours
#Function 3: Pivot Table
#User chooses whether they would like to view a pivot table of price or destination, showing Uber vs. Lyft rides for each source/destination 
combo
def pivot()
:
 
values = st.radio("View Price or Distance Data:", (
'price', 'distance'
)
)
 
pivTable = pd.pivot_table(df, values=values, index=[
'source', 'destination'
], columns='cab_type', aggfunc=np.sum)
...
'Hour', 'Day', 'Month', 'Datetime', 'Source', 'Destination', 'Cab 
Type', 'Name', 'Price', 'Distance', 'Temperature', 'Weather'
], [
'Day', 'Cab Type'
]
)
...
columns = [
'Hour', 'Day', 'Month', 'Datetime', 'Source', 'Destination', 'Cab Type', 'Name', 'Price', 'Distance', 'Temperature', 'Weather'
]
 
month = st.radio("Select month:", (
'November', 'December'
)
)
...
'Select day', options=[1, 2, 3, 4, 9, 10, 13, 14, 15, 16, 17, 18]
)
...
day = st.select_slider(
'Select day', options=[26, 27, 28, 29, 30]
)
...
monthDay = df_one[
(df_one.Month == mon) & (df_one.Day == day)
]
...
monthDay = df_one[
(df_one.Month == mon) & (df_one.Day == day) & (df_one.Temperature > temp)
]
 
else: monthDay = df_one[
(df_one.Month == mon) & (df_one.Day == day) & (df_one.Temperature > temp) & (df_one.Destination == dest)
]
...
countmeanmed = st.sidebar.radio("Plot the line based on:", (
'Count', 'Mean', 'Median', 'Mean & Median'
)
)
 
color = st.sidebar.color_picker(
'Pick A Line Color', '#B9159D'
)
...
meanormed = st.selectbox("Mean or Median:", (
'mean', 'median'
)
)
 
selectedColumn = st.selectbox("Choose a column to get the mean of:", (
'price', 'distance'
)
)
